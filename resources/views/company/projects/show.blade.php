<x-company-layout :title="$project->name">
    ...
    <div x-data="{ tab: 'list' }">
        ...
        <div x-show="tab==='list'">
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:12px;">
                ...
                <div class="al-toolbar" style="display:flex; align-items:center; gap:2px;">
                    {{-- Filter --}}
                    <div style="position:relative;">
                        ...
                        <div id="tbFilter" class="tb-menu" style="width:480px; max-width:92vw; padding:18px 20px;">
                            ...
                            <div style="display:flex; flex-wrap:wrap; gap:8px; margin-top:8px;">
                                <button class="qf-pill" data-qf="incomplete" onclick="tbQuick('incomplete')">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/></svg> Incomplete tasks
                                </button>
                                <button class="qf-pill" data-qf="completed" onclick="tbQuick('completed')">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M8 12l3 3 5-6"/></svg> Completed tasks
                                </button>
                                <button class="qf-pill" data-qf="mine" onclick="tbQuick('mine')">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="3.5"/><path d="M5 20a7 7 0 0114 0"/></svg> Just my tasks
                                </button>
                                <button class="qf-pill" data-qf="due_this_week" onclick="tbQuick('due_this_week')">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="17" rx="2"/><path d="M3 9h18M8 2v4M16 2v4"/></svg> Due this week
                                </button>
                                <button class="qf-pill" data-qf="due_next_week" onclick="tbQuick('due_next_week')">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg> Due next week
                                </button>
                            </div>
                            {{-- active filter groups render here --}}
                    ...
                    {{-- Sort --}}
                    <div style="position:relative;">
                        <button class="tb-btn" onclick="tbToggle(event,'tbSort')">
                    ...
                    {{-- Group --}}
                    <div style="position:relative;">
                        <button class="tb-btn" onclick="tbToggle(event,'tbGroup')">
                    ...
                    {{-- Options --}}
                    <div style="position:relative;">
                        <button class="tb-btn" onclick="tbToggle(event,'tbOptions')">
    ...
    function rowVisible(row){
        ...
        // due filters (OR within due keys)
        const dueKeys=['due_this_week','due_next_week','overdue','no_date'].filter(k=>TB.qf.has(k));
        if(dueKeys.length){
            const due=row.dataset.due, today=fmtDate(new Date());
            const ok = dueKeys.some(k =>
                (k==='due_this_week' && dueInWeek(due,0)) ||
                (k==='due_next_week' && dueInWeek(due,1)) ||
                (k==='overdue' && due && due<today && row.dataset.status!=='done') ||
                (k==='no_date' && !due)
            );
            if(!ok) return false;
        }
        // assignee (OR within selected assignees)
        if(TB.assignee.size){
            const ids=(row.dataset.assignees||'').split(',');
            if(![...TB.assignee].some(a=>ids.includes(String(a)))) return false;
        }
    ...
    function tbQuick(key){
        if(TB.qf.has(key)){ TB.qf.delete(key); }
        else {
            if(key==='incomplete') TB.qf.delete('completed');
    ...
    function renderFilterGroups(){
        const wrap=document.getElementById('tbFilterGroups');
    ...
    function tbFilterChange(type, val, on){
        const set = type==='status' ? TB.status : TB.priority;
    ...
    function tbSetGroup(mode){
        const inner=document.querySelector('.al-table-inner');
        if(mode==='none') inner.classList.add('group-none'); else inner.classList.remove('group-none');
    }
    ...
    function tbSetSort(key){
        document.querySelectorAll('.al-tasklist').forEach(list=>{
            ...
            rows.sort((a,b)=>{
                ...
                if(key==='modified') return (b.dataset.modified||'').localeCompare(a.dataset.modified||''); // most recent first
            ...
            rows.forEach(row=>{
                ...
                const subs=document.getElementById('subs-'+row.id.replace('row-',''));
                if(subs) list.appendChild(subs);
            });
            const addRow=list.querySelector(':scope > .al-addrow');
            if(addRow) list.appendChild(addRow);
        });
    }
    // remember original order for "Default" sort
