...
@unless(request()->routeIs('company.projects.show', 'employee.projects.show'))
...
{{-- Global Search Modal --}}
<div id="searchModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.7); z-index:500; align-items:flex-start; justify-content:center; padding-top:80px;">
    <div style="background:var(--surface); border:1px solid var(--border2); border-radius:14px; width:100%; max-width:580px; box-shadow:0 20px 60px rgba(0,0,0,0.4); overflow:hidden;">
        {{-- Input --}}
        <div style="display:flex; align-items:center; gap:12px; padding:14px 18px; border-bottom:1px solid var(--border);">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" stroke-width="2" style="flex-shrink:0;"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input id="searchInput" type="text" placeholder="Search tasks, projects, people..." autocomplete="off"
                style="flex:1; background:transparent; border:none; outline:none; color:var(--text); font-family:var(--font); font-size:14px;"
                oninput="debounceSearch(this.value)">
            <span style="font-size:11px; color:var(--muted); font-family:var(--mono); cursor:pointer;" onclick="closeSearch()">ESC</span>
        </div>
        {{-- Results --}}
        <div id="searchResults" style="max-height:420px; overflow-y:auto; padding:8px 0;">
            <div style="padding:32px; text-align:center; color:var(--muted); font-size:13px;">Type to search...</div>
        </div>
    </div>
</div>
...
<script>
let searchTimer;
const searchUrl = '/{{ $slug }}/admin/search';
...
function closeSearch() {
    ...
    document.getElementById('searchResults').innerHTML = '<div style="padding:32px; text-align:center; color:var(--muted); font-size:13px;">Type to search...</div>';
}
...
function debounceSearch(val) {
    ...
    document.getElementById('searchResults').innerHTML = '<div style="padding:32px; text-align:center; color:var(--muted); font-size:13px;">Searching...</div>';
...
function renderResults(data, q) {
    ...
    if (data.total === 0) {
        el.innerHTML = '<div style="padding:32px; text-align:center; color:var(--muted); font-size:13px;">No results for "' + q + '"</div>';
        return;
    }

    let html = '';

    const statusColor = s => ({ done:'#4ade80', in_progress:'#22d3ee', in_review:'#a78bfa' }[s] || 'var(--muted)');
    const statusLabel = s => ({ done:'Done', in_progress:'In Progress', in_review:'In Review', todo:'To Do' }[s] || s);

    if (data.results.tasks.length) {
        html += `<div style="padding:6px 16px 4px; font-size:10px; color:var(--muted); font-family:var(--mono); text-transform:uppercase; letter-spacing:0.08em;">Tasks</div>`;
        data.results.tasks.forEach(t => {
            html += `<a href="${t.url}" onclick="closeSearch()" style="display:flex; align-items:center; gap:12px; padding:9px 16px; text-decoration:none; transition:background 0.1s;" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background='transparent'">
                <div style="width:28px; height:28px; border-radius:7px; background:rgba(34,211,238,0.1); display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#22d3ee" stroke-width="2"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                </div>
                <div style="flex:1; min-width:0;">
                    <div style="font-size:13px; color:var(--text); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${t.title}</div>
                    <div style="font-size:11px; color:var(--muted); font-family:var(--mono);">${t.subtitle}</div>
                </div>
                <span style="font-size:10px; font-family:var(--mono); padding:2px 7px; border-radius:5px; color:${statusColor(t.status)}; background:${statusColor(t.status)}18; border:1px solid ${statusColor(t.status)}33; white-space:nowrap;">${statusLabel(t.status)}</span>
            </a>`;
        });
    }

    if (data.results.projects.length) {
        html += `<div style="padding:10px 16px 4px; font-size:10px; color:var(--muted); font-family:var(--mono); text-transform:uppercase; letter-spacing:0.08em; border-top:1px solid var(--border);">Projects</div>`;
        data.results.projects.forEach(p => {
            html += `<a href="${p.url}" onclick="closeSearch()" style="display:flex; align-items:center; gap:12px; padding:9px 16px; text-decoration:none; transition:background 0.1s;" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background='transparent'">
                <div style="width:28px; height:28px; border-radius:7px; background:rgba(74,222,128,0.1); display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#4ade80" stroke-width="2"><path d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z"/></svg>
                </div>
                <div style="flex:1; min-width:0;">
                    <div style="font-size:13px; color:var(--text); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${p.title}</div>
                    <div style="font-size:11px; color:var(--muted); font-family:var(--mono); text-transform:capitalize;">${p.subtitle}</div>
                </div>
            </a>`;
        });
    }

    if (data.results.members.length) {
        html += `<div style="padding:10px 16px 4px; font-size:10px; color:var(--muted); font-family:var(--mono); text-transform:uppercase; letter-spacing:0.08em; border-top:1px solid var(--border);">People</div>`;
        data.results.members.forEach(m => {
            html += `<a href="${m.url}" onclick="closeSearch()" style="display:flex; align-items:center; gap:12px; padding:9px 16px; text-decoration:none; transition:background 0.1s;" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background='transparent'">
                <div style="width:28px; height:28px; border-radius:7px; background:rgba(167,139,250,0.15); color:#a78bfa; font-size:12px; font-weight:600; display:flex; align-items:center; justify-content:center; flex-shrink:0;">${m.initial}</div>
                <div style="flex:1; min-width:0;">
                    <div style="font-size:13px; color:var(--text);">${m.title}</div>
                    <div style="font-size:11px; color:var(--muted); font-family:var(--mono);">${m.subtitle}</div>
                </div>
            </a>`;
        });
    }
